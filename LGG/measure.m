function [P, R, ACC, F1, AUC] = measure(pre_lab, label, pre_p)
[m] = confusionmat(label, pre_lab);
Pt = diag(m) ./ sum(m)';           % 查准率
Rt = diag(m) ./ sum(m, 2);         % 查全率
Pt(isnan(Pt)) = [];
Rt(isnan(Rt)) = [];
P = mean(Pt);
R = mean(Rt);
ACC = sum(diag(m)) / sum(sum(m)); % ACC;
F1 = (2 * P * R) / (P + R); 
AUC = 0;
if length(unique(label)) == 2 && ~isempty(pre_p)
    AUC = plot_roc(pre_p, label); % AUC
end
end
...
% OUTPUTS
%  auc            - 返回ROC曲线的曲线下的面积
ground_truth = (ground_truth - min(ground_truth)) ./ (max(ground_truth) - min(ground_truth));
